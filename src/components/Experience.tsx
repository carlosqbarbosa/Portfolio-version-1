...
const careerData = [
  ...
  return (
    <Container id="experience" className="section page-container">
      <motion.div
        initial="hidden"
        whileInView="show"
        variants={fadeUp}
      ...
      >
        ...
        <Typography variant="h4" align="center" sx={{ fontWeight: 700, mt: 1, mb: 6 }}>
          Minha Trajetória & Experiência
        </Typography>
      </motion.div>
            ...
            >
              <Stack direction="row" spacing={2} sx={{ mb: 1, alignItems: 'center' }}>
                {job.highlight ? (
                  ...
                  <CircleIcon sx={{ color: 'text.secondary', fontSize: 8 }} />
                )}
                <Box>
                  <Typography variant="h6" sx={{ fontWeight: 600 }}>{job.role}</Typography>
                  <Typography variant="body2" color="text.secondary">
                    {job.company} | {job.period}
                  </Typography>
                </Box>
              </Stack>
              <Stack spacing={1.2} sx={{ pl: 4, mb: 3 }}>
                {job.responsibilities.map((item, i) => (
                    ...
                    >
                    <Box sx={{ color: 'primary.main', fontSize: 10, mt: "6px" }}>•</Box>
                    {item}
                    </Typography>
                ))}
                </Stack>
        ...
        >
          ...
          <Stack spacing={1.5}>
            {keySkills.map((skill, i) => (
              <Typography key={i} sx={{ fontSize: 16, display: "flex", alignItems: "center", gap: 1 }}>
                <Box sx={{ color: 'primary.main', fontSize: 16 }}>&gt;</Box>
                {skill}
              </Typography>
            ))}
          </Stack>
